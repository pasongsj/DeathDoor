...
namespace GameEngineDebug
{
	...
	void DrawCapsule(class GameEngineCamera* _Cam, class GameEngineTransform* _Trans, float4 Color)
	{
		if (DebugDrawDatas.end() == DebugDrawDatas.find(_Cam))
		{
			DebugDrawDatas[_Cam].reserve(1000);
		...
		DebugDrawDatas[_Cam].push_back({ DebugDrawType::Capsule, _Trans, Color });
	}
