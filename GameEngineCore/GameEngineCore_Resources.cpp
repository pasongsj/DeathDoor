...
void GameEngineCore::CoreResourcesInit()
{
	...
	// Capsule
		// 캡슐
		{
			...
			float Radius = 0.5f;
			// 북극점부터 시작합니다.
			V.POSITION = float4(0.0f, Radius*2.f, 0.0f, 1.0f);
			V.TEXCOORD = float4(0.5f, 0.0f);
			// 노말 백터 혹은 법선백터라고 불리며
			// 면에 수직인 벡터를 의미하게 된다.
			// 빛을 반사할때 필수.
			...
			for (UINT y = 1; y < iStackCount; ++y)
			{
				...
				for (UINT z = 0; z < iSliceCount + 1; ++z)
				{
					float theta = z * zRotAngle;
					V.POSITION = float4{
						Radius * sinf(y * yRotAngle) * cosf(z * zRotAngle)*0.5f,
						Radius * cosf(y * yRotAngle)*2.f,
						Radius * sinf(y * yRotAngle) * sinf(z * zRotAngle) * 0.5f,
						1.0f // 위치 크기 값에 영향을 주기 위해서
					};
